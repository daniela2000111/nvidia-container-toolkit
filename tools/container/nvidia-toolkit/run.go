...
const (
	runDir         = "/run/nvidia"
	defaultPidFile = "/run/nvidia/toolkit/toolkit.pid"
...
func main() {
	...
	c.Flags = []cli.Flag{
		...
		&cli.StringFlag{
			...
			Usage:       "the path to a .pid file to ensure that only a single configuration instance is running",
...
func validateFlags(_ *cli.Context, o *options) error {
	if filepath.Base(o.pidFile) != "toolkit.pid" {
